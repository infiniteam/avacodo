...
package class Rule003500 extends Rule {
	...
	override package applyTo(RichIbanResult it, BankConfig config, LocalDate date) {
		if(account.bankCode==790_200_76 && !config.deletionAnnounced){
			...
			creationNotPossible("bank code must be 790_200_76 and not be announced for deletion "+account.bankCode)
		}
	}
}