...
describe Rule003000 {
	...
	facts "all can be converted"{
		examples.forEach[
			try{
				subject.applyTo(new LegacyAccount(13091054,it),"XX".config).doubleCheck.iban should not be null
			} catch(AccountValidationException e){
				fail("account valid should be suppressed "+ it)
			}
		]
	}
}